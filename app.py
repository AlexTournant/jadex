...
def streetform():
    ...
    if form.validate_on_submit():
        ...
        if pokemon_id != -1:
            ...
            return render_template('show.html', info=pokemon_info, form=form, pokemon_names=pokemon_names,nom=nom,est_connecte=est_connecte,inCollection=inCollection,id_pokemon=pokemon_id)
...
def login():
    ...
    return (render_template('login.html',pokemon_names=pokemon_names,formulaire=formulaire,form=form,message=message,nom=nom,est_connecte=est_connecte))
...
def register():
    ...
    if formulaire.validate_on_submit():
        ...
        id=ID_selon_mdp_et_nom(formulaire.nom.data,formulaire.mdp.data)
        return redirect('/')
    return render_template('register.html',pokemon_names=pokemon_names,formulaire=formulaire,form=form,est_connecte=est_connecte,nom=nom)
...
def collection():
    ...
    return (render_template('collection.html',form=form,collection=tab,pokemon_names=pokemon_names,nom=nom,est_connecte=est_connecte))
